...
class FinanceInvoiceController extends Controller
{
    ...
    public function finance_invoice($status='')
    {
        if($status==5){
            $status='';
        }
        $invoice=DB::table('create_invoices')->select('create_invoices.*', 'clear_logs.consignee_name','customers.*','companies.*') // , 'customers.*','companies.*'
            ->leftJoin('clear_logs','create_invoices.log_clear_id','clear_logs.id')
            ->leftJoin('customers','clear_logs.customer_id','customers.id')
            ->leftJoin('companies','customers.company_id','companies.id')
           // ->join('containers','containers.id','pgl_invoices.container_id')
            ->orderBy('create_invoices.id','desc')
            ->get();

//       dd($invoice);
        if($status !=''){
            $invoice->where('create_invoices.status',$status);
        }
        $invoices=$invoice->paginate(20);
        return view('admin.finance.invoice',['invoices'=>$invoices,'paginate'=>20,'status'=>$status]);
    }
