...
=======
		   		<form action="{{route('on_theway_vehicle_admin')}}" id="showEntryForm">
...
>>>>>>> parent of affd84d (Cleared the repo)
		   ...
		   <div class="col-md-2 col-lg-2 col-sm-6 col-xs-12 text-right" style="margin-top:1.5%;float: right;text-align: right;">
		   	<a href="#" class="text text-warning"><b>On The Way Vehicles</b></a>
		   </div>
	<div class="site" id="user_data">
		@include('admin.vehicle.on_theway_vehicles_data')
	</div>
...
=======
	   	$(document).on('click','.paginatio a',function(e){
>>>>>>> parent of affd84d (Cleared the repo)
	   		e.preventDefault();
...
=======
	   		// getMoreVehicle(1);
	   		window.location.href="{{route('on_theway_vehicle_admin')}}";
>>>>>>> parent of affd84d (Cleared the repo)
	   	});
	   	 function getMoreVehicle(page){
		       ...
		       var request = $.ajax({
	              url: "{{route('paginate_on_theway_vehicle_admin')}}" +'?page='+page,
	              method: "GET",
       ...
       $('#search').on('keyup',function(e){
	   	 ...
	   	 function searchVehicle(searchData){
	      	  $('#searchBody').html("<div style='position:fixed; margin-top:7%; margin-left:40%;'><img width='70px' src='img/loading.gif' alt='Loading ...'> </div> ");
		       var request = $.ajax({
	              url: "{{route('search_on_theway_vehicle_admin')}}",
	              method: "GET",
...
<<<<<<< HEAD
       	 var data = $(this).val();
       		var request = $.ajax({
	              url: "{{route('paginate_entry_on_theway_vehicle_admin')}}",
	              method: "GET",
	              data: {paginate:data},
	            }); 
	            request.done(function( msg ) {
