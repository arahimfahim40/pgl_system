...
@section('title','Pending Vehicles')
@section('content')
...
=======
		   		<form action="{{route('pending_vehicle_admin')}}" id="showEntryForm">
...
=======
	   	$(document).on('click','.pagiation a',function(e){
>>>>>>> parent of affd84d (Cleared the repo)
	   		e.preventDefault();
...
<<<<<<< HEAD
	   		getMoreVehicle(1);
=======
	   		window.location.href="{{route('pending_vehicle_admin')}}";
>>>>>>> parent of affd84d (Cleared the repo)
	   	});
	   	 function getMoreVehicle(page){
		       ...
		       var request = $.ajax({
	              url: "{{route('paginate_pending_vehicle_admin')}}" +'?page='+page,
	              method: "GET",
       ...
       $('#search').on('keyup',function(e){
	   	 ...
	   	 function searchVehicle(searchData){
	      	  $('#searchBody').html("<div style='position:fixed; margin-top:7%; margin-left:40%;'><img width='70px' src='img/loading.gif' alt='Loading ...'> </div> ");
		       var request = $.ajax({
	              url: "{{route('search_pending_vehicle_admin')}}",
	              method: "GET",
...
<<<<<<< HEAD
       	 var data = $(this).val();
       		var request = $.ajax({
	              url: "{{route('paginate_entry_pending_vehicle_admin')}}",
	              method: "GET",
	              data: {paginate:data},
	            }); 
	            request.done(function( msg ) {
