...
<div class="site-content">
  <div class="content-area py-1">
	...
	<div class="site" id="user_data">
		@include('customer.shipment.all_shipment_data')
	</div>
</div>
@stop
@section('js')
	...
	$(document).ready(function(){
		// pagination section
	   	$(document).on('click','.agination a',function(e){
	   	 ...
	   	 function getMoreVehicle(page){
		       ...
		       var request = $.ajax({
	              url: "{{route('paginate_shipment_base_location_and_status_customer')}}" +'?page='+page,
	              method: "GET",
       ...
       $('#search').on('keyup',function(e){
	   	 ...
	   	 function searchVehicle(searchData){
	      	$('#searchBody').html("<div style='position:fixed; margin-top:7%; margin-left:40%;'><img width='70px' src= '"+"{{asset('img/loading.gif')}}"+"' alt='Loading ...'> </div> ");
		       var request = $.ajax({
	              url: "{{route('shipment_base_location_and_status_search_customer')}}",
	              method: "GET",
	              data: {searchValue:searchData,status:"{{$status}}",location:"{{$location}}"},
	            }); 
	            request.done(function( msg ) {
	                $('#user_data').html(msg);
	            });
       ...
       $('#showEntry').change(function(){
       	 ...
       	 var data = $(this).val();
       		var request = $.ajax({
	              url: "{{route('paginate_shipment_base_location_and_status_customer')}}",
	              method: "GET",
	              data: {paginate:data,status:"{{$status}}",location:"{{$location}}"},
	            }); 
	            request.done(function( msg ) {
	                $('#user_data').html(msg);
	            });
