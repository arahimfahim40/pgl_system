...
<div class="site-content">
  <div class="content-area py-1">
	...
	<div class="site" id="user_data">
		@include('customer.vehicle.onhand_notitle_vehicles_data')
	</div>
</div>
@stop
@section('js')
	...
	$(document).ready(function(){
		// pagination section
	   	$(document).on('click','.pagination a',function(e){
	   	 ...
	   	 function getMoreVehicle(page){
		       ...
		       var request = $.ajax({
	              url: "{{route('paginate_onhand_notitle_vehicle_customer')}}" +'?page='+page,
	              method: "GET",
       ...
       $('#search').on('keyup',function(e){
	   	 ...
	   	 function searchVehicle(searchData){
	      	  $('#searchBody').html("<div style='position:fixed; margin-top:7%; margin-left:40%;'><img width='70px' src='img/loading.gif' alt='Loading ...'> </div> ");
		       var request = $.ajax({
	              url: "{{route('search_onhand_notitle_vehicle_customer')}}",
	              method: "GET",
       ...
       $('#showEntry').change(function(){
       	 ...
       	 var data = $(this).val();
       		var request = $.ajax({
	              url: "{{route('paginate_entry_onhand_notitle_vehicle_customer')}}",
	              method: "GET",
	              data: {paginate:data},
	            }); 
	            request.done(function( msg ) {
